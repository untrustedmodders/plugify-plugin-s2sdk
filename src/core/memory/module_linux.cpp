...
void CModule::BuildFunctionIndexAndReferences()
{
	...
	std::vector<const Section*> data_segments;
	data_segments.reserve(_sections.size());
	...
	for (const auto& section : _sections)
	{
        if (section.flags & FLAG_X)
        {
            if (!exec_start)
            {
                exec_start = section.address;
                exec_end	= section.address + section.size;
                exec_size  = section.size;
            }
        }
        else
        {
            data_segments.push_back(&section);
            min_data_addr = std::min(min_data_addr, section.address);
            max_data_addr = std::max(max_data_addr, section.address + section.size);
        }
	}
	...
	auto is_data_pointer = [&](uintptr_t addr) noexcept {
        if (addr < min_data_addr || addr >= max_data_addr)
            return false;
        return std::ranges::any_of(data_segments, [addr](const Section* s) noexcept {
	...
	for (const Section* seg : data_segments)
	{
