...
public:
	...
	plg::string MenuButtonBodyFontClass;					// Panorama CSS class (e.g. "fontSize-sm", "fontSize-s") wrapping the WASD button menu's body
	...
	plg::string MenuButtonFooterFontStyle;					// Same, but wrapping just the Up/Down/Left/Right/Exit control row; empty keeps the default
															// (unscaled) footer size
